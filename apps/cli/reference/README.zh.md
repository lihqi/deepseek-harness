...
基础组合包挂载原生 DeepSeek 会话适配器、settings 与凭据提供方、隔离的 Codex 驱动 `web_search`、仅限公网的 HTTP fetch 提供方，以及按反馈门控的会话遥测。会话提供方凭据依次从继承环境、`$DSH_HOME/.credentials.yaml`、调用目录的 `.env` 和 `$DSH_HOME/.env` 解析；受管文档从不物化进 `process.env`，而两个 `.env` 文件都是普通启动环境层。搜索则通过提供方 id `codex-local` 使用已配置本地 Codex 账户的普通 `auth.json`，不会读取 `DEEPSEEK_API_KEY` 或 `DEEPSEEK_SEARCH_BASE_URL`。每次搜索都在独立空 workspace 与私有 home 中运行包内固定 Codex payload，并以认证文件作为唯一文件桥接；完整隔离与查询前拒绝规则见 [`dsh-web-search-codex`](../../../packages/web/web-search-codex/README.zh.md)。Web app 的 `cordis`、`ptc` 与 `standard` agent preset 会在所有 sandbox 和审批模式下暴露 `web_fetch`，无需逐次确认；提供方仍会在连接前拒绝非公开目的地址。
